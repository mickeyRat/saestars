...
var sid = $.cookie('SID');
// ================================= 2020 ========================================
function sae_viewPublishedResults(){
    var eventIDX = $.cookie('LOCATION');
    // console.log('eventIX = '+eventIDX);
    $.ajax({
        type: 'POST',
        url: '../cgi-bin/results.pl',
        data: {'do':'sae_viewPublishedResults','act':'print','eventIDX':eventIDX},
...
function sae_openCreateResults(){
    $.modal('Generate Results', '400px');
    var location = $.cookie('LOCATION');
    $.ajax({
        type: 'POST',
        url: '../cgi-bin/results.pl',
        data: {'do':'sae_openCreateResults','act':'print','location':location},
        success: function(str){
            // $(obj).replaceWith(str);
...
function sae_changeResultType(value){
    if (value == 17 || value==10){
        $('.sae-roundSelection').removeClass('w3-hide');
    } else {
        $('.sae-roundSelection').addClass('w3-hide');
        $("#resultRound").val($("#resultRound option:first").val());
    }
}
function sae_generateResults(o){
    var resultType = $('#resultType option:selected').val();
    var resultClass = $('#resultClass option:selected').val();
    var resultRound = $('#resultRound option:selected').val();
    var location = $.cookie('LOCATION');
    $.ajax({
        type: 'POST',
        url: '../cgi-bin/results.pl',
        data: {'do':'sae_generateResults','act':'print','location':location,'resultType':resultType,'resultRound':resultRound,'resultClass':resultClass},
        success: function(str){
            // console.log(str);
            $('#TABLE_RESULTS_'+resultClass).prepend(str);
            // $(o).close();
        }
    });
}
function sae_deleteResults(publishIDX){
    var jsYes = confirm("Are you sure?");
    if (!jsYes){return}
    $.ajax({
        type: 'POST',
        url: '../cgi-bin/results.pl',
        data: {'do':'sae_deleteResults','act':'print','publishIDX':publishIDX},
        success: function(str){
            $('#TR_RESULT_'+publishIDX).remove();
        }
    });
}
function sae_updateResultAttribute(o, field, publishIDX){
    var value = 0;
    if ($(o).is(':checked')){value=1}
    $.ajax({
        type: 'POST',
        url: '../cgi-bin/results.pl',
        data: {'do':'sae_updateResultAttribute','act':'print','publishIDX':publishIDX,'value':value,'field':field},
        success: function(str){}
    });
}
// ================================= 2020 ========================================
