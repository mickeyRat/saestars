...
sub _getAvailableJudges(){
    ...
    my $limit = 3;
    my @EXTRA = ();
    my $SQL = "SELECT U.PK_USER_IDX, U.BO_EXTRA FROM TB_USER AS U JOIN TB_PREF AS P ON U.PK_USER_IDX=P.FK_USER_IDX WHERE P.FK_EVENT_IDX=?";
    ...
    print "Total # of Available Judges = ".@JUDGES."\n";
    foreach $userIDX (sort keys %USERS) {
        if ($USERS{$userIDX}{BO_EXTRA}==1){
            push(@EXTRA, $userIDX);
            next;
        }
    ...
    print "# of EXTRA Judges = ".@EXTRA."\n\n";

    #Perform the Limit Assignment

    foreach $teamIDX (sort {$a <=> $b} keys %TEAMS) {
        my $count = $TCARD{$teamIDX}+0;
        if ($count==3){print "\n"; next}
        my @TJUDGE = @JUDGES;
        until ($count>2) {
            my $availableJudges = @TJUDGE;
            my $index = int(1+rand($availableJudges))-int(1);
            my $userIDX = $TJUDGE[$index];
            
            
            if ($availableJudges==0){last}
            if ($userIDX){
                 $CARD{$userIDX}++;
                 $TCARD{$teamIDX}++;
