...
sub _getTotalPenalty(){
    my ($self, $teamIDX) = @_;
    my $SQL = "SELECT sum(IN_DEDUCTION) AS IN_ECR FROM TB_ECR WHERE FK_TEAM_IDX=?";
    my $select = $dbi->prepare($SQL);
	   $select->execute( $teamIDX );
	my ($inDeductions) = $select->fetchrow_array();
	return ($inDeductions);
}
sub _getECRS(){
    my ($self, $teamIDX) = @_;
    my $SQL = "SELECT * FROM TB_ECR WHERE FK_TEAM_IDX=?";
    my $select = $dbi->prepare($SQL);
	   $select->execute( $teamIDX );
