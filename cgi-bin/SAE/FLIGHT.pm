...
sub _getDensityAltitude(){
    ...
    my $upperLimit = $epoch * 1.000000098570300;
    my $SQL = "SELECT IN_DENSITY from TB_WEATHER 
        where (IN_EPOCH >= ? and IN_EPOCH < ? and FK_EVENT_IDX=?) order by abs(IN_EPOCH - ? ) limit 1";
    my $select = $dbi->prepare($SQL);
    $select->execute( $lowerLimit, $upperLimit, $eventIDX, $epoch);
    my ($densityAltitude) = $select->fetchrow_array(); 
...
sub _getUsedTicketCount(){
    ...
    # my $SQL = "SELECT F.FK_TEAM_IDX, F.IN_STATUS, COUNT(F.FK_TEAM_IDX) AS IN_USED FROM TB_FLIGHT AS F JOIN TB_TEAM AS T ON T.PK_TEAM_IDX=F.FK_TEAM_IDX  where F.FK_EVENT_IDX=? GROUP BY F.FK_TEAM_IDX, F.IN_STATUS ORDER BY `F`.`FK_TEAM_IDX` ASC";
    my $select = $dbi->prepare($SQL);
...
sub _getFlightTickets(){
    my $self = shift;
    my $teamIDX = shift;
    my $SQL = "SELECT TEAM.FK_CLASS_IDX, TEAM.BO_REINSPECT AS TEAM_INSPECT_STATUS, FLT.* FROM TB_FLIGHT AS FLT 
        JOIN TB_TEAM AS TEAM ON FLT.FK_TEAM_IDX=TEAM.PK_TEAM_IDX WHERE FLT.FK_TEAM_IDX=?";
    my $select = $dbi->prepare($SQL);
        $select->execute($teamIDX);
    my %HASH = %{$select->fetchall_hashref('PK_FLIGHT_IDX')}; 
    return (\%HASH);
}
sub _getFlightTicketDetails(){
    my $self = shift;
    my $flightIDX = shift;
    my $SQL = "SELECT TEAM.FK_CLASS_IDX, TEAM.BO_REINSPECT AS TEAM_INSPECT_STATUS, TEAM.IN_NUMBER, TEAM.IN_STD, FLT.* FROM TB_FLIGHT AS FLT JOIN TB_TEAM AS TEAM ON FLT.FK_TEAM_IDX=TEAM.PK_TEAM_IDX WHERE FLT.PK_FLIGHT_IDX=?";
    my $select = $dbi->prepare($SQL);
        $select->execute($flightIDX);
    my %HASH = %{$select->fetchrow_hashref()}; 
    return (\%HASH);
...
sub _getReasonsNoScore(){
    my $self = shift;
    my $SQL = "SELECT * FROM TB_REASON";
    my $select = $dbi->prepare($SQL);
        $select->execute();
    my %HASH = %{$select->fetchall_hashref('TX_REASON')}; 
    my @LIST = sort {$a cmp $b} keys %HASH ;
    return (\@LIST);
}
sub _getInspectionItems(){
    my ($self, $flightIDX) = @_;
    my $SQL = "SELECT CL_ITEMS FROM TB_REINSPECT WHERE (FK_FLIGHT_IDX=? AND BO_STATUS=?)";
    my $select = $dbi->prepare($SQL);
        $select->execute($flightIDX, 1);
    my $ITEMS = $select->fetchrow_array(); 
    # print $ITEMS;
    return ($ITEMS);
}
sub _getReinspectionList(){
    my ($self, $flightIDX) = @_;
    my $SQL = "SELECT * FROM TB_REINSPECT WHERE (FK_FLIGHT_IDX=? AND BO_STATUS=?)";
    my $select = $dbi->prepare($SQL);
        $select->execute($flightIDX, 1);
    my %HASH = %{$select->fetchrow_hashref()}; 
    return (\%HASH);
}
sub _getInspectionList(){
    my ($self) = @_;;
...
sub _getTeamInspectionLogs(){
    my ($self, $teamIDX) = @_;
    my $SQL = "SELECT * FROM TB_REINSPECT WHERE (FK_TEAM_IDX=?)";
    my $select = $dbi->prepare($SQL);
        $select->execute($teamIDX);
    my %HASH = %{$select->fetchall_hashref('PK_REINSPECT_IDX')}; 
    return (\%HASH);
}
sub _getReinspectKey (){
    my ($self, $flightIDX, $boStatus) = @_;
    my $SQL = "SELECT PK_REINSPECT_IDX FROM TB_REINSPECT WHERE (FK_FLIGHT_IDX=? AND BO_STATUS=?)";
    my $select = $dbi->prepare($SQL);
        $select->execute($flightIDX, $boStatus);
    my $inspectIDX = $select->fetchrow_array(); 
    return ($inspectIDX);
    }
...
sub _getTicketDetails(){
    my ($self, $eventIDX, $teamIDX, $inRound) = @_;
    my $SQL = "SELECT * FROM TB_FLIGHT WHERE (FK_EVENT_IDX=? and FK_TEAM_IDX=? and IN_ROUND=?)";
    my $select = $dbi->prepare($SQL);
        $select->execute($eventIDX, $teamIDX, $inRound);
    my %HASH = %{$select->fetchrow_hashref()}; 
    return (\%HASH);
}
sub _getFlightInspectionStatus(){
    my ($self, $flightIDX) = @_;shift;
    my $SQL = "SELECT BO_REINSPECT FROM TB_FLIGHT WHERE PK_FLIGHT_IDX=?";
    my $select = $dbi->prepare($SQL);
        $select->execute($flightIDX);
    my $boReinspect = $select->fetchrow_array(); 
    return ($boReinspect);
}
# ===============================================================
# INSERT
