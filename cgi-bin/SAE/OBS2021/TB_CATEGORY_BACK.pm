...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_CATEGORY_IDX, TX_TYPE, TX_CATEGORY, IN_VALUE FROM TB_CATEGORY_BACK";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_CATEGORY_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_TxType(){
	...
	my $SQL = "DELETE FROM TB_CATEGORY_BACK where TX_TYPE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxCategory(){
	...
	my $SQL = "DELETE FROM TB_CATEGORY_BACK where TX_CATEGORY=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_InValue(){
	...
	my $SQL = "DELETE FROM TB_CATEGORY_BACK where IN_VALUE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub updatePkCategoryIdx_ById(){
	...
	my $SQL = "UPDATE TB_CATEGORY_BACK SET PK_CATEGORY_IDX=? where PK_CATEGORY_IDX = ?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return ();
}
