...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_CLASS_IDX, TX_CLASS FROM TB_CLASS";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_CLASS_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_TxClass(){
	...
	my $SQL = "DELETE FROM TB_CLASS where TX_CLASS=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub updatePkClassIdx_ById(){
	my ($self) = shift;
	my ($value) = shift;
	my ($idx) = shift;

	my $SQL = "UPDATE TB_CLASS SET PK_CLASS_IDX=? where PK_CLASS_IDX = ?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return ();
}
