...
sub deleteRecordBy_TxSerial(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_UPLOAD where TX_SERIAL=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxFile(){
	...
	my $SQL = "DELETE FROM TB_UPLOAD where TX_FILE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxLabel(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_UPLOAD where TX_LABEL=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_InSize(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_UPLOAD where IN_SIZE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
