...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_USER_TEAM_IDX, FK_USER_IDX, FK_TEAM_IDX FROM TB_USER_TEAM";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_USER_TEAM_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_PkUserTeamIdx(){
	my ($self) = shift;
	my ($idx) = shift;
...
sub deleteRecordBy_FkUserIdx(){
	...
	my $SQL = "DELETE FROM TB_USER_TEAM where FK_USER_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_FkTeamIdx(){
	...
	my $SQL = "DELETE FROM TB_USER_TEAM where FK_TEAM_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub updatePkUserTeamIdx_ById(){
	my ($self) = shift;
	my ($value) = shift;
	my ($idx) = shift;

	my $SQL = "UPDATE TB_USER_TEAM SET PK_USER_TEAM_IDX=? where PK_USER_TEAM_IDX = ?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return ();
}
