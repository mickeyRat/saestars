...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_USER_TYPE_IDX, TX_TYPE, IN_TYPE FROM TB_USER_TYPE";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_USER_TYPE_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_PkUserTypeIdx(){
	my ($self) = shift;
	my ($idx) = shift;
...
sub deleteRecordBy_TxType(){
	...
	my $SQL = "DELETE FROM TB_USER_TYPE where TX_TYPE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_InType(){
	...
	my $SQL = "DELETE FROM TB_USER_TYPE where IN_TYPE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub updatePkUserTypeIdx_ById(){
	my ($self) = shift;
	my ($value) = shift;
	my ($idx) = shift;

	my $SQL = "UPDATE TB_USER_TYPE SET PK_USER_TYPE_IDX=? where PK_USER_TYPE_IDX = ?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return ();
}
