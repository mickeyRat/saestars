...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_QUESTION_IDX, TX_TITLE, TX_DESCRIPTION, IN_PARENT, IN_WEIGHT FROM TB_QUESTION";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_QUESTION_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_InParent(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_QUESTION where IN_PARENT=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
