...
sub deleteRecordBy_PkScoreEventIdx(){
	my ($self) = shift;
	my ($idx) = shift;
...
sub deleteRecordBy_TxScoreEvent(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_SCORE_EVENT where TX_SCORE_EVENT=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_InMax(){
	...
	my $SQL = "DELETE FROM TB_SCORE_EVENT where IN_MAX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
