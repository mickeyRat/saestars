...
sub addNewRecord(){
	...
	my $SQL = "INSERT INTO TB_TODO (FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	my $insert = $dbi->prepare($SQL);
	   $insert->execute($FkTeamIdx, $FkTodoTypeIdx, $FkEventIdx, $FkFlightIdx, $ClDescription, $ClComment, $TxTime, $TxRoom, $TxStatus, $InClear);
...
sub getRecordById(){
	my ($self) = shift;
	my ($idx) = shift;
	$SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR
		FROM TB_TODO
		WHERE PK_TODO_IDX=?";
...
sub getAllRecordBy_PkTodoIdx(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE PK_TODO_IDX=?";
	my $select = $dbi->prepare($SQL);
...
sub getAllRecordBy_FkTeamIdx(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE FK_TEAM_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_FkTodoTypeIdx(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE FK_TODO_TYPE_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_FkEventIdx(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE FK_EVENT_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_FkFlightIdx(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE FK_FLIGHT_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_ClDescription(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE CL_DESCRIPTION=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_ClComment(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE CL_COMMENT=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxTime(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE TX_TIME=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxRoom(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE TX_ROOM=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxStatus(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE TX_STATUS=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_InClear(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO WHERE IN_CLEAR=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_TODO_IDX, FK_TEAM_IDX, FK_TODO_TYPE_IDX, FK_EVENT_IDX, FK_FLIGHT_IDX, CL_DESCRIPTION, CL_COMMENT, TX_TIME, TX_ROOM, TX_STATUS, IN_CLEAR FROM TB_TODO";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_TODO_IDX'])};
	return(\%HASH);
