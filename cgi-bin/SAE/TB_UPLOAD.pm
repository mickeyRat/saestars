...
sub addNewRecord(){
	...
	my $SQL = "INSERT INTO TB_UPLOAD (FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE) values (?, ?, ?, ?, ?, ?, ?, ?)";
	my $insert = $dbi->prepare($SQL);
	   $insert->execute($FkTeamIdx, $FkEventIdx, $TxKeys, $TxFilename, $TxType, $TxPaper, $InPaper, $BlFile);
...
sub getRecordById(){
	my ($self) = shift;
	my ($idx) = shift;
	$SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE, TS_CREATE
		FROM TB_UPLOAD
		WHERE PK_UPLOAD_IDX=?";
...
sub getAllRecordBy_PkUploadIdx(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE PK_UPLOAD_IDX=?";
	my $select = $dbi->prepare($SQL);
...
sub getAllRecordBy_FkTeamIdx(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE FK_TEAM_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_FkEventIdx(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE FK_EVENT_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxKeys(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE TX_KEYS=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxFilename(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE TX_FILENAME=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxType(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE TX_TYPE=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxPaper(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE TX_PAPER=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_InPaper(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE IN_PAPER=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_BlFile(){
	my ($self) = shift;
	my ($value) = shift;

	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD WHERE BL_FILE=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_UPLOAD_IDX, FK_TEAM_IDX, FK_EVENT_IDX, TX_KEYS, TX_FILENAME, TX_TYPE, TX_PAPER, IN_PAPER, BL_FILE FROM TB_UPLOAD";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_UPLOAD_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_BlFile(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_UPLOAD where BL_FILE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub getIdBy_BlFile(){
	...
	$SQL = "SELECT PK_UPLOAD_IDX from TB_UPLOAD where BL_FILE=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my $idx = $select->fetchrow_array();
...
sub updateBlFile_byId(){
	my ($self) = shift;
	my ($value) = shift;
	my ($idx) = shift;

	my $SQL = "UPDATE TB_UPLOAD set BL_FILE=? where PK_UPLOAD_IDX=?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return($value);
