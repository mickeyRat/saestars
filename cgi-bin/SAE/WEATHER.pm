...
sub _getWeatherByHours(){
    ...
    my $hours = shift;
    my $limit = $hours * 12;
    my $SQL = "SELECT PK_WEATHER_IDX, IN_TEMP, IN_PRES, IN_RH, IN_ELEVATION, IN_DENSITY, DATE_FORMAT(TS_LOCAL, '%r - %W %c/%e') AS TS_LOCAL, TS_CREATE, IN_EPOCH FROM TB_WEATHER WHERE FK_EVENT_IDX=? ORDER BY PK_WEATHER_IDX DESC LIMIT ?";
