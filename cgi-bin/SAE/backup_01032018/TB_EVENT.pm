...
sub addNewRecord(){
	...
	my $SQL = "INSERT INTO TB_EVENT (TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB) values (?, ?, ?, ?, ?, ?)";
	my $insert = $dbi->prepare($SQL);
	   $insert->execute($TxEventName, $InYear, $InElevation, $TxEventCity, $BoArchive, $TxWeb);
...
sub getRecordById(){
	my ($self) = shift;
	my ($idx) = shift;
	$SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB
		FROM TB_EVENT
		WHERE PK_EVENT_IDX=?";
...
sub getAllRecordBy_PkEventIdx(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE PK_EVENT_IDX=?";
	my $select = $dbi->prepare($SQL);
...
sub getAllRecordBy_TxEventName(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE TX_EVENT_NAME=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_InYear(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE IN_YEAR=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_InElevation(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE IN_ELEVATION=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxEventCity(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE TX_EVENT_CITY=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_BoArchive(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE BO_ARCHIVE=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxWeb(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT WHERE TX_WEB=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_EVENT_IDX, TX_EVENT_NAME, IN_YEAR, IN_ELEVATION, TX_EVENT_CITY, BO_ARCHIVE, TX_WEB FROM TB_EVENT";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_EVENT_IDX'])};
	return(\%HASH);
