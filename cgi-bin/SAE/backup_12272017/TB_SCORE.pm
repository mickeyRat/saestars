...
sub addNewRecord(){
	...
	my $SQL = "INSERT INTO TB_SCORE (FK_TEAM_IDX) values (?)";
	my $insert = $dbi->prepare($SQL);
	   $insert->execute($FkTeamIdx);
...
sub getAllRecordBy_PkScoreIdx(){
	...
	my $SQL = "SELECT PK_SCORE_IDX, FK_TEAM_IDX FROM TB_SCORE WHERE PK_SCORE_IDX=?";
	my $select = $dbi->prepare($SQL);
...
sub getAllRecordBy_FkTeamIdx(){
	...
	my $SQL = "SELECT PK_SCORE_IDX, FK_TEAM_IDX FROM TB_SCORE WHERE FK_TEAM_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_SCORE_IDX'])};
	return(\%HASH);
