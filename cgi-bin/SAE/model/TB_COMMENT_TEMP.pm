...
sub deleteRecordBy_PkCommentTemIdx(){
	my ($self) = shift;
	my ($idx) = shift;
...
sub deleteRecordBy_FkUserIdx(){
	...
	my $SQL = "DELETE FROM TB_COMMENT_TEMP where FK_USER_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxTitle(){
	...
	my $SQL = "DELETE FROM TB_COMMENT_TEMP where TX_TITLE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxComment(){
	...
	my $SQL = "DELETE FROM TB_COMMENT_TEMP where TX_COMMENT=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}

sub deleteRecordBy_BoPrivate(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_COMMENT_TEMP where BO_PRIVATE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
