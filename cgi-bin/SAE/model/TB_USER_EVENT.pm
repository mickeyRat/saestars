...
sub deleteRecordBy_PkUserEventIdx(){
	my ($self) = shift;
	my ($idx) = shift;
...
sub deleteRecordBy_FkUserIdx(){
	...
	my $SQL = "DELETE FROM TB_USER_EVENT where FK_USER_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_FkEventIdx(){
	...
	my $SQL = "DELETE FROM TB_USER_EVENT where FK_EVENT_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_InLimit(){
	...
	my $SQL = "DELETE FROM TB_USER_EVENT where IN_LIMIT=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
