...
sub _login(){
    ...
    if ($pkUserIdx && $storedPassword eq $saltedPassword) {                  # if user ID exists and the stored Password matches 
        ...
        $pkUserIdx = $storedPassword = $boReset = $txFirst = $txLast = $txType = '';
    }
    $DATA{AUTH} = $auth;
    $DATA{PK_USER_IDX} = $pkUserIdx;
    $DATA{SESSION_ID} = $sid;
    $DATA{BO_RESET} = $boReset;
...
sub _cryptPassword(){
    my $self = shift;
    my $password = shift;
    my $salt = &getRandomText(2);
    my $saltedPassword = $salt.crypt($password, $salt);
    return ($saltedPassword);
#begin
#* Object: _cryptPassword()
#* Inputs: my ($cryptPassword) = $obj->_cryptPassword();
#* Output: returns the encrypted password provided by the user readable password.
#* Comment: none
}
## --------- SETTERS ----------------------------------------------
...
# sub _getTempPassword(){
#     my ($self, $email) = @_;
#     my $User = new LRD::TB_USER();
#     my $Pwd = new LRD::TB_PWD();
#     $pkUserIDX = $User->_getIdBy_TxEmail($email);
#     my $tempPassword = &getTempPassword(64);
#     my $cryptPassword = &cryptPassword($tempPassword);
#     $Pwd->_setField(FK_USER_IDX,$pkUserIDX);
#     $Pwd->_setField(TX_HASH,$cryptPassword);
#     $Pwd->_saveNew();
# }
