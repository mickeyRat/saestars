...
sub _deleteRow(){
	...
	my $where  = shift; 
	my $idx    = shift; 
	my $SQL = "DELETE FROM TB_CARD WHERE $where=?";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
}
sub _deleteAll(){
	my $self   = shift; 
	my $SQL = "DELETE FROM TB_CARD";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute();
}
...
sub _fetchData(){
	...
	my $where  = shift; 
	my $idx  = shift; 
	my $SQL = "SELECT * FROM TB_CARD WHERE $where=?"; 
...
sub _getData_hashref(){
	...
	my $select; 
	if ($where){ 
		$SQL = "SELECT * FROM TB_CARD WHERE $where=?"; 
...
sub _saveUpdate(){
	...
	push (@setVal, $idx);
	my $SQL = "UPDATE TB_CARD SET $fields WHERE PK_CARD_IDX=?";
...
sub _saveNew(){ 
	...
	my $SQL = "INSERT INTO TB_CARD (".join(", ",@FIELDS).") values (".$PARAMS.")"; 
