...
sub _deleteRow(){
	...
	my $where  = shift; 
	my $idx    = shift; 
	my $SQL = "DELETE FROM TB_PWD WHERE $where=?";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
}
sub _deleteAll(){
	my $self   = shift; 
	my $SQL = "DELETE FROM TB_PWD";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute();
}
...
sub _fetchData(){
	...
	my $where  = shift; 
	my $idx  = shift; 
	my $SQL = "SELECT * FROM TB_PWD WHERE $where=?"; 
...
sub _getData_hashref(){
	...
	my $select; 
	if ($where){ 
		$SQL = "SELECT * FROM TB_PWD WHERE $where=?"; 
		$select = $dbi->prepare($SQL);
		$select->execute($idx);
	} else { 
		$SQL = "SELECT * FROM TB_PWD"; 
		$select = $dbi->prepare($SQL);
		$select->execute();
	} 
...
sub _saveNew(){ 
	...
	my $SQL = "INSERT INTO TB_PWD (".join(", ",@FIELDS).") values (".$PARAMS.")"; 
