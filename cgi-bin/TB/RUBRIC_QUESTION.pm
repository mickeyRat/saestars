...
my $PkRubricQuestionIdx;
my $FkRubricSectionIdx;
my $InWeight;
my $InOrder;
my %DATA = ();
my %FIELDS = ();
...
sub _deleteRow(){
	...
	my $where  = shift; 
	my $idx    = shift; 
	my $SQL = "DELETE FROM TB_RUBRIC_QUESTION WHERE $where=?";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
}
sub _deleteAll(){
	my $self   = shift; 
	my $SQL = "DELETE FROM TB_RUBRIC_QUESTION";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute();
}
...
sub _fetchData(){
	...
	my $where  = shift; 
	my $idx  = shift; 
	my $SQL = "SELECT * FROM TB_RUBRIC_QUESTION WHERE $where=?"; 
...
sub _getData_hashref(){
	...
	my $select; 
	if ($where){ 
		$SQL = "SELECT * FROM TB_RUBRIC_QUESTION WHERE $where=?"; 
		$select = $dbi->prepare($SQL);
		$select->execute($idx);
	} else { 
		$SQL = "SELECT * FROM TB_RUBRIC_QUESTION"; 
		$select = $dbi->prepare($SQL);
		$select->execute();
	} 
...
sub _saveUpdate(){
	...
	push (@setVal, $idx);
	my $SQL = "UPDATE TB_RUBRIC_QUESTION SET $fields WHERE PK_RUBRIC_QUESTION_IDX=?";
...
sub _saveNew(){ 
	...
	my $SQL = "INSERT INTO TB_RUBRIC_QUESTION (".join(", ",@FIELDS).") values (".$PARAMS.")"; 
