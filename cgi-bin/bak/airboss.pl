...
exit;
sub sae_saveFlightLineNotes(){
    print $q->header();
    my $eventIDX = $q->param('eventIDX');
    my $teamIDX = $q->param('teamIDX');
    my $txNotes = uri_unescape($q->param('txNotes'));
    my $Airboss = new SAE::AIRBOSS();
    $Airboss->_saveNotes($txNotes, $teamIDX, $eventIDX);
    my $str = sprintf '<p class="w3-margin-bottom w3-border-bottom" style="padding: 10px;"><b class="w3-text-green">Flight-Line Notes:</b><br>%s</p>', $txNotes;
    return ($str);
}
sub sae_openTeamNotesList(){
    print $q->header();
    my $eventIDX = $q->param('eventIDX');
    my $Airboss = new SAE::AIRBOSS($eventIDX);
    my %TEAMS = %{$Airboss->_getTeamData()};
    
    my $str = '<div class=" w3-container w3-margin-top w3-row"><br>';
    $str .= '<h3>Flight Notes</h3>';
    $str .= '<div class="w3-row">';
    foreach $teamIDX (sort {$TEAMS{$a}{IN_NUMBER} <=> $TEAMS{$b}{IN_NUMBER}} keys %TEAMS) {
        my $flag = $Airboss->_getNoteFlag($teamIDX);
        my $class = 'w3-white ';
        if ($flag == 1){$class='w3-yellow '}
        $str .= sprintf '<div ID="AB_%d" class="%s w3-container w3-bar w3-mobile w3-display-container w3-round w3-border w3-button w3-margin-bottom w3-left-align w3-padding-8" onclick="sae_viewFlightNotes(%d);">', $teamIDX, $class, $teamIDX;
        $str .= sprintf '<span class="w3-bar-item w3-xxlarge w3-display-left"  style="text-shadow:1px 2px 0 #444;"><b>%03d</b></span>', $TEAMS{$teamIDX}{IN_NUMBER};
        $str .= sprintf '<p class="w3-mobile" style="margin-left: 50px; padding: 20px; display:table;">%s</p>', $TEAMS{$teamIDX}{TX_SCHOOL};
        # $str .= '<i class="fa fa-chevron-right w3-display-right w3-margin-right"></i>';
        if ($flag == 1){
            $str .= sprintf '<i ID="ABC_%d" class="fa fa-commenting-o fa-2x w3-display-right w3-bar-item w3-transparent w3-round" ></i>', $teamIDX;
        } else {
            $str .= sprintf '<i ID="ABC_%d" class="fa fa-commenting-o fa-2x w3-display-right w3-bar-item w3-transparent w3-round" style="display:none;"></i>', $teamIDX;
        }
        $str .= '</div>';
    }
    $str .= '</div>';
...
sub sae_viewFlightNotes(){
    ...
    my %NOTES = %{$Airboss->_getNotes($teamIDX)};
    my $str;
    $str = '<div class="w3-contianer" style="height: 550px; overflow-y: auto;">';
    $str .= sprintf '<div class="w3-center"><span class="w3-small">%s</span><br><span class="w3-xxlarge" style="text-shadow:1px 2px 0 #444;">%03d</span></div>', $TEAM{TX_SCHOOL}, $TEAM{IN_NUMBER};
    $str .= '<button id="btn_addNotes" class="w3-button w3-border w3-round w3-mobile w3-green w3-large w3-hover-blue" onclick="sae_showAddFlightNotes(this);">+ Notes<i class="fa fa-commenting-o fa-fw w3-margin-left" aria-hidden="true"></i></button>';
    $str .= '<div ID="divAddNotes" class="w3-container w3-round w3-margin-bottom w3-border-bottom" style="display:none;">';
    $str .= '<h3 class="w3-center">Notes</h3>';
    $str .= '<textarea ID="txt_FlightNotes" class="w3-border w3-input w3-round w3-large" style="resize:vertical; height: 100px;" placeholder="Flight Notes"></textarea>';
    $str .= '<div class="w3-center w3-margin-bottom w3-margin-top">';
    $str .= sprintf '<button class="w3-button w3-border w3-round w3-margin-right w3-blue" onclick="sae_saveFlightLineNotes(%d);">Save</button>', $teamIDX;
    $str .= '<button class="w3-button w3-border w3-round w3-margin-left w3-red" onclick="sae_cancelShowAddNotes();">Cancel</button>';
    $str .= '</div>';
    $str .= '</div>';
    
    $str .= '<div ID="DIV_NOTES_LIST">';
    if (scalar(keys %NOTES)>0){
        foreach $item (sort {$b<=>$a} keys %NOTES) {
            $str .= sprintf '<p class="w3-margin-bottom w3-border-bottom w3-large" style="padding: 10px;">%s</p>', $NOTES{$item};
        }
    } else {
        $str .= sprintf '<p class="w3-margin-bottom w3-border-bottom" style="padding: 10px;">No notes for this team/aircraft</p>', ;
    }
    $str .= '</div>';
    $str .= '</div>';
    return ($str);
...
#     if ($inType == 1){
#         $Flight->_updateFlightButton($teamIDX, $btn);
#         return ($btn);
#     } else {
#         return;
#     }
...
#             $str .= sprintf '<option value="%d" '.$selected.'>%s</option>', $hourIDX, $HOURS{$hourIDX}{TS_CREATE};
#         }
#         $str .= '</select>';
#         $str .= '</div>';
...
#         # $str .= '<div class="w3-col l1 w3-center"><span class="w3-hide-large w3-left">Payload</span>';
#         # $str .= sprintf '<input  tabindex="'.($tabIndex++).'" class="w3-input w3-border w3-round w3-right" step="0.1" min="0"  type="number" placeholder="0.0" value="%s"  onchange="sae_updateFlightCardField(%d, this.value, \'%s\', %d);">',$Logs->getInWeight(),$flightIDX, 'IN_WEIGHT', $teamIDX ;
#         # $str .= sprintf '<button class="w3-display-right" style="margin-right: 2px; padding: 4px;" onclick="sae_openCalculator(%d, \'WEIGHT_\');"><i class="fa fa-calculator w3-pale-green" aria-hidden="true"></i></button>', $flightIDX;
#         # $str .= '</div>';
...
#         if ($flightIDX == $fltIDX){
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-green w3-hover-blue w3-card-2 w3-round w3-small"       style="width: 70px; margin-right: 4px;" onclick="sae_updateFlightStatus(this, %d, %d, %d, %d, %d, %d);">Good</button>',   $flightIDX, 1, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-light-grey w3-hover-blue w3-card-2 w3-round w3-small"  style="width: 70px; margin-right: 4px;" onclick="sae_updateFlightStatus(this, %d, %d, %d, %d, %d, %d);">No Fly</button>', $flightIDX, 2, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-orange w3-hover-blue w3-card-2 w3-round  w3-small"     style="width: 70px; margin-right: 4px;" onclick="sae_updateFlightStatus(this, %d, %d, %d, %d, %d, %d);">Crash</button>',  $flightIDX, 3, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-pale-yellow w3-hover-yellow w3-card-2 w3-round w3-small" style="width: 70px; margin-right: 4px;" onclick="sae_showFlightNotes(%d);">Notes</button>', $flightIDX;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-white w3-hover-red w3-round w3-small" style="width: 70px;" onclick="sae_deleteFlightLog(this, %d, %d, %d, %d, %d, %d);">Cancel</button>', $flightIDX, 1, $teamIDX, $inNumber, $classIDX, $inRound;
#         } else {
#             $str .= sprintf '<select ID="flightState_'.$flightIDX.'" class="w3-select w3-margin-right w3-border w3-round" style="width: 50%; height: 40px; padding: 3px;" onchange="sae_updateFlightState(this, %d, %d, %d, %d, %d);">', $flightIDX, $teamIDX, $inNumber, $classIDX, $inRound;
...
#             $str .= '</select>';
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-pale-yellow w3-hover-yellow w3-card-2 w3-round w3-small" style="width: 70px; margin-right: 4px;" onclick="sae_showFlightNotes(%d);">Notes</button>', $flightIDX;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-white w3-hover-red w3-round w3-small" style="width: 70px;" onclick="sae_deleteFlightLog(this, %d, %d, %d, %d, %d, %d);">Delete</button>', $flightIDX, 0, $teamIDX, $inNumber, $classIDX, $inRound;
#         }
...
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-white w3-hover-red w3-round w3-small" style="width: 70px;" onclick="sae_deleteFlightLog(this, %d, %d, %d, %d, %d, %d);">Cancel</button>', $flightIDX, 1, $teamIDX, $inNumber, $classIDX, $inRound;
#         } else {
#             $str .= sprintf '<select ID="flightState_'.$flightIDX.'" class="w3-select w3-margin-right w3-border w3-round w3-padding" style="width: 50%; height: 40px;" onchange="sae_updateFlightState(this, %d, %d, %d, %d, %d);">', $flightIDX, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<option value="0" selected>No Status</option>';
#             foreach $inStatus (sort {$a<=>$b} keys %STATUS){
#                 my $selected = '';
#                 if ($Status == $inStatus){$selected = 'selected'}
...
#         my $Status = $Logs->getInStatus();
#         if ($flightIDX == $fltIDX){
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-green w3-hover-blue w3-card-2 w3-round w3-small"         style="width: 60px; margin-right: 4px;" onclick="sae_updateFlightStatus(this, %d, %d, %d, %d, %d, %d);">Good</button>',   $flightIDX, 1, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-light-grey w3-hover-blue w3-card-2 w3-round w3-small"    style="width: 60px; margin-right: 4px;" onclick="sae_updateFlightStatus(this, %d, %d, %d, %d, %d, %d);">No Fly</button>', $flightIDX, 2, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-orange w3-hover-blue w3-card-2 w3-round  w3-small"       style="width: 60px; margin-right: 4px;" onclick="sae_updateFlightStatus(this, %d, %d, %d, %d, %d, %d);">Crash</button>',  $flightIDX, 3, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-pale-yellow w3-hover-yellow w3-card-2 w3-round w3-small" style="width: 60px; margin-right: 4px;" onclick="sae_showFlightNotes(%d);">Notes</button>', $flightIDX;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-white w3-hover-red w3-round w3-small"                    style="width: 60px;" onclick="sae_deleteFlightLog(this, %d, %d, %d, %d, %d, %d);">Cancel</button>', $flightIDX, 1, $teamIDX, $inNumber, $classIDX, $inRound;
#         } else {
#             $str .= sprintf '<select ID="flightState_'.$flightIDX.'" class="w3-select w3-margin-right w3-border w3-round w3-padding w3-small" style="width: 35%; height: 40px;" onchange="sae_updateFlightState(this, %d, %d, %d, %d, %d);">', $flightIDX, $teamIDX, $inNumber, $classIDX, $inRound;
#             $str .= sprintf '<option value="0" selected>No Status</option>';
#             foreach $inStatus (sort {$a<=>$b} keys %STATUS){
#                 my $selected = '';
#                 if ($Status == $inStatus){$selected = 'selected'}
...
#             $str .= '</select>';
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-pale-yellow w3-hover-yellow w3-card-2 w3-round w3-small" style="width: 60px; margin-right: 4px;" onclick="sae_showFlightNotes(%d);">Notes</button>', $flightIDX;
#             $str .= sprintf '<button tabindex="'.($tabIndex++).'" class="w3-button w3-border w3-white w3-hover-red w3-round w3-small" style="width: 60px;" onclick="sae_deleteFlightLog(this, %d, %d, %d, %d, %d, %d);">Delete</button>', $flightIDX, 0, $teamIDX, $inNumber, $classIDX, $inRound;
#         }
...
#     my $inFlightStatus = 1;
#     my $btn = $Flight->_tempCheckOutButton($teamIDX, $inFlightStatus);
#     if($reInspection){
#         $inFlightStatus = 3;
#         $btn = $Flight->_templateReinspectionButton($flightIDX, $teamIDX, $inRound, $inFlightStatus);
#     }
...
#     my $flightIDX = $q->param('flightIDX');
#     $Flight->_updateFlightField($flightIDX, 'CL_NOTES', $inValue, $teamIDX);
#     return ();
# }
# sub sae_showFlightNotes(){
#     print $q->header();
#     my $Flight = new SAE::TB_FLIGHT();
#     my $Util = new SAE::Common();
#     my $flightIDX = $q->param('flightIDX');
#     $Flight->getRecordById($flightIDX);
#     my $Comments = $Flight->getClNotes();
#     my $str;
    
#     my $notes = $Util->removeBr($Comments);
#     $str = '<dic class="w3-container">';
#     $str .= '<label>Notes</lable>';
#     $str .= sprintf '<textarea ID="NOTES_'.$flightIDX.'" class="w3-input w3-border w3-round" style="max-width: 100%; height: 150px;">%s</textarea><br><br>', $notes;
#     $str .= sprintf '<div class="w3-display-container"><button class="w3-button w3-border w3-round w3-card-2 w3-display-middle" onclick="sae_saveFlightNotes(this, %d, \'%s\');">Save</button></div>', $flightIDX, 'CL_NOTES';
#     $str .= '</div>';
#     return ($str);
# }
# ##======================== Penalties =====================================================
...
#     my %CRASH = %{$Flight->_getCrashInspectionSheet($flightIDX)};
#     # %CRASH = %{$Flight->_getCrashDataByTeamId($teamIDX)};
#     my $str = '<div class="w3-container w3-padding">';
#     $str .= '<div class="w3-panel">';
#     my @ITEMS = split(";", $CRASH{CL_DESCRIPTION});
#     $str .= sprintf '<div class="w3-bar-item w3-large">%s', $CRASH{TX_STATUS};
#         $str .= '<ul class="w3-ul">Item(s):';
#         foreach $item (sort {$a<=>$b} @ITEMS){
#             $str .= '<li class="w3-small" style="padding: 1px; margin-left: 20px">'.$item.'</li>';
#         }
#         $str .= '</ul>';
#     $str .= '</div>';
#     $str .= '<div class="w3-bar-item ">';
#     $str .= '<label>Crash Notes</label><br>';
#     $str .= '<p class="w3-padding w3-round w3-border">'.$CRASH{CL_COMMENT}."</p>";
#     $str .= '</div>';
#     $str .= '<center class="w3-margin-top">';
#     $str .= sprintf '<button class="w3-button w3-border w3-green w3-bar-item w3-round" onclick="sae_clearReinspection(this, %d, %d, %d, \'%s\')">Clear</button>', $flightIDX, $teamIDX, $classIDX, $txNumber;
#     $str .= '</center>';
#     $str .= '</div>';
#     return($str);
# }
...
#     my $inFlightStatus = 1;
#     my $Flight = new SAE::FLIGHT();
#     $Flight->_updateReinspectionItems( $teamIDX, $flightIDX );
#     my $btn = $Flight->_tempCheckOutButton($teamIDX, $inFlightStatus);
#     $Flight->_updateFlightButton($teamIDX, $inFlightStatus, $btn);
#     return ($btn);
...
#     my $btn = sprintf '<button class="w3-white w3-hover-blue w3-button w3-border w3-round-large " onclick="$(this).close();sae_recordFlightCard(this, 0, %d, \'%03d\', 0);">View Flight Logs</button>',$teamIDX, $inNumber;
#     if ($classIDX==2){
#         $btn = sprintf '<button class="w3-white w3-hover-blue w3-button w3-border w3-round-large " onclick="$(this).close();sae_recordFlightCard_Adv(this, 0, %d, \'%03d\', 0);">View Flight Logs</button>',$teamIDX, $inNumber;
#     }
#     if ($classIDX==3){
#         $btn = sprintf '<button class="w3-white w3-hover-blue w3-button w3-border w3-round-large " onclick="$(this).close();sae_recordFlightCard_Mic(this, 0, %d, \'%03d\', 0);">View Flight Logs</button>',$teamIDX, $inNumber;
#     }
#     my $str = '<div class="w3-margin w3-padding w3-display-container" style="height: 125px;">';
#     $str .= '<label for="sae_roundSelection">Flight Round</label>';
...
#     }
#     $str .= '</select>';
#     $str .= '<div class="w3-container w3-display-bottomright">';
    
#     $str .= $btn;
#     $str .= '<button class="w3-green w3-hover-blue w3-button w3-border w3-round-large w3-card-2 w3-margin-left" style="width: 120px;" ';
#     $str .= sprintf 'onclick="sae_checkoutFlightLog(this, %d, %d, %d);">', $teamIDX, $classIDX, $inNumber;
#     $str .= 'Checkout</button>';
#     $str .= '</div>';
#     $str .= '</div>';
#     return($str);
...
#     # my $btn = &_templateCheckInButton_Reg($flightIDX, $teamIDX, $inRound, 1);
#     if ($classIDX==2) {      ## Advanced Class
#         $btn = $Flight->_templateCheckInButton_Adv($flightIDX, $teamIDX, $inRound, $inFlightStatus);
#         # $btn = &_templateCheckInButton_Adv($flightIDX, $teamIDX, $inNumber, $inRound);
#         # $str = sprintf '<button class="w3-button w3-border w3-round w3-green w3-hover-blue" style="margin-top: 5px;" onclick="sae_openAdvCheckoutCard(%d, \'%s\', %d);">#%s</button>', $teamIDX, $number, $classIDX, $number.': Flight Card In';
#     } elsif ($classIDX==3) { ## Micro Class
#         $btn = $Flight->_templateCheckInButton_Mic($flightIDX, $teamIDX, $inRound, $inFlightStatus);
#         # $btn = &_templateCheckInButton_Mic($flightIDX, $teamIDX, $inNumber, $inRound);
#         # $str = sprintf '<button class="w3-button w3-border w3-round w3-green w3-hover-blue" style="margin-top: 5px;" onclick="sae_openCheckoutCard(%d, \'%s\', %d);">#%s</button>', $teamIDX, $number, $classIDX, $number.': Flight Card In';
#     } 
