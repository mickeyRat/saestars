...
sub sae_viewPublishedResults(){
    ...
    print $q->header(); 
    my $str;
    $str .= '<div class="w3-container w3-margin-top w3-white"><br><br>';
    $str .= '<h2>Results</h2>';
    foreach $classIDX (sort keys %CLASS) {
        my %LIST = %{$Scores->_getPublishedListByClass($eventIDX, $classIDX)};
        $str .= '<div class="w3-container">';
        $str .= sprintf '<h3>%s</h3>', $CLASS{$classIDX};
        $str .= '<ul class="w3-ul">';
        foreach $publishIDX (sort {$LIST{$a}{IN_ROUND} <=> $LIST{$b}{IN_ROUND}} keys %LIST) {
