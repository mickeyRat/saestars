...
my @TYPE = (0, 1, 2, 3, 4, 99);
my @TILES = (29, 30, 31);

foreach $inType (@TYPE) {
    my %USERS = %{$User->_getUserByType($inType)};
    my %ACCESS = %{$User->_getUserAccess($inType)};
    foreach $userIDX (sort keys %USERS) {
        foreach $tileIDX (@TILES){
            if (!exists $ACCESS{$userIDX}{$tileIDX} ){
                printf "%10d Missing $tileIDX Access\n", $userIDX;
