...
sub _deleteRow(){
	...
	my $where  = shift; 
	my $idx    = shift; 
	my $SQL = "DELETE FROM TB_QUESTION WHERE $where=?";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
}
sub _deleteAll(){
	my $self   = shift; 
	my $SQL = "DELETE FROM TB_QUESTION";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute();
}
...
sub _fetchData(){
	...
	my $where  = shift; 
	my $idx  = shift; 
	my $SQL = "SELECT * FROM TB_QUESTION WHERE $where=?"; 
...
sub _getData_hashref(){
	...
	my $select; 
	if ($where){ 
		$SQL = "SELECT * FROM TB_QUESTION WHERE $where=?"; 
...
sub _saveUpdate(){
	...
	push (@setVal, $idx);
	my $SQL = "UPDATE TB_QUESTION SET $fields WHERE PK_QUESTION_IDX=?";
...
sub _saveNew(){ 
	...
	my $SQL = "INSERT INTO TB_QUESTION (".join(", ",@FIELDS).") values (".$PARAMS.")"; 
