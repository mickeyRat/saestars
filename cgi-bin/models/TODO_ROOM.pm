...
sub _deleteRow(){
	...
	my $where  = shift; 
	my $idx    = shift; 
	my $SQL = "DELETE FROM TB_TODO_ROOM WHERE $where=?";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
}
sub _deleteAll(){
	my $self   = shift; 
	my $SQL = "DELETE FROM TB_TODO_ROOM";  
	my $delete = $dbi->prepare($SQL);
	   $delete->execute();
}
...
sub _fetchData(){
	...
	my $where  = shift; 
	my $idx  = shift; 
	my $SQL = "SELECT * FROM TB_TODO_ROOM WHERE $where=?"; 
...
sub _getData_hashref(){
	...
	my $select; 
	if ($where){ 
		$SQL = "SELECT * FROM TB_TODO_ROOM WHERE $where=?"; 
		$select = $dbi->prepare($SQL);
		$select->execute($idx);
	} else { 
		$SQL = "SELECT * FROM TB_TODO_ROOM"; 
		$select = $dbi->prepare($SQL);
		$select->execute();
	} 
...
sub _saveUpdate(){
	...
	push (@setVal, $idx);
	my $SQL = "UPDATE TB_TODO_ROOM SET $fields WHERE PK_TODO_ROOM_IDX=?";
...
sub _saveNew(){ 
	...
	my $SQL = "INSERT INTO TB_TODO_ROOM (".join(", ",@FIELDS).") values (".$PARAMS.")"; 
