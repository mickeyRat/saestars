...
#= 2024 =============================================================================
    sub paper_openAutoAssign (){
        ...
        $str .= '<label># of Judges per team</label>';
        $str .= '<input ID="CLASS_LIMIT" type="number" value="3" class="w3-input w3-border w3-round w3-margin-bottom" style="width: 110px; margin-left: auto; margin-right: auto; text-align: center">';
        $str .= sprintf '<button class="w3-button w3-border w3-round w3-hover-green w3-pale-green w3-margin-top" onclick="paper_autoAssign(this, %d, %d);">Generate Auto-Assignment</button>', $classIDX, $inType;
        $str .= '</div>';
        return ($str);
        }
    sub paper_autoAssign (){
        ...
        my %BUTTON     = %{$Paper->_autoAssign($eventIDX, $classIDX, $inLimit, $inType)};
        my $json = encode_json \%BUTTON;
        return ($json);
        }
    sub paper_showVolunteerStatistics (){
        print $q->header();
        my $eventIDX  = $q->param('eventIDX');
        ...
        my %SCORE     = %{$Paper->_getVolunteerStatistics($eventIDX , $userIDX, $classIDX )};
        my $str; 
        foreach $teamIDX (sort keys %SCORE) {
            my %TEAMSCORE = %{$Grade->_getCardAssessmentWeightedScores('Assessment', $teamIDX)};
            my %TEAM      = %{$Team->_getTeamDetails($teamIDX)};
            my $inNumber = $TEAM{IN_NUMBER};
            my $txSchool = $TEAM{TX_SCHOOL};
            $str .= sprintf '<tr class="w3-khaki w3-small vol_%d">', $userIDX;
            $str .= sprintf '<td><span class="w3-margin-left" style="margin-left: 30px !important;"> %03d-%s</span></td>', $inNumber, $txSchool;
            foreach $inSec (sort {$a <=> $b} keys %{$SCORE{$teamIDX}}) {
                foreach $inSub (sort {$a <=> $b}  keys %{$SCORE{$teamIDX}{$inSec}}) {
                    my $inValue = $SCORE{$teamIDX}{$inSec}{$inSub}{IN_VALUE};
                    $cardIDX = $SCORE{$teamIDX}{$inSec}{$inSub}{PK_CARD_IDX};
                    $str .= sprintf '<td style="text-align: center"><i>%d</i></td>', $inValue;
                }
            }

            $str .= sprintf '<td><i>%2.2f</i></td>', $TEAMSCORE{$cardIDX}{IN_SCORE};
            $str .= '</tr>';
        }

        return ($str);
        }
    sub paper_getTeamScoringDetails (){
        print $q->header();
        ...
        my %RUBRIC = %{$Rubric->_getRubric($TYPE{$inType}, $classIDX)};
        ...
        my $str;
        # $str = join("-<br> ", keys %SCORE);
        $str .= '<div class="w3-responsive">';
        $str .= '<table class="w3-table-all" style="width: 100%">';
        $str .= '<thead>';
        $str .= '<tr class="w3-small w3-blue-grey">';
        $str .= '<td>Judge/Section</td>';
        foreach $inSec (sort {$a <=> $b} keys %RUBRIC) {
            foreach $inSub (sort {$a <=> $b} keys %{$RUBRIC{$inSec}}) {
                $str .= sprintf '<td class="w3-center w3-border" style="width: 40px;">%d.%0.2d</td>', $inSec , $inSub;
            }
        } 
        $str .= '<td style="text-align: right; width: 50px">Score</td>';
        $str .= '</tr>';
        $str .= '</thead>';
        $str .= '<tbody>';
        my @WSCORE = ();
        foreach $cardIDX (sort keys %VOL) {
            ...
            $str .= sprintf '<td class="w3-small"><a href="javascript:void(0);" onclick="paper_showVolunteerStatistics(this, %d, %d, %d, %d);">%s</a></td>', $userIDX, $classIDX, $teamIDX, $cardIDX, $VOL{$cardIDX}{TX_LAST_NAME} ;
            foreach $inSec (sort {$a <=> $b} keys %RUBRIC) {
                foreach $inSub (sort {$a <=> $b} keys %{$RUBRIC{$inSec}}) {
                    my $reportIDX = $CARDS{$cardIDX}{$inSec}{$inSub}{PK_REPORT_IDX};
                    my $inScore   = $CARDS{$cardIDX}{$inSec}{$inSub}{IN_VALUE};
                    my $scoreBG   = 'w3-white w3-text-black';
                    if ($inScore == 0){$scoreBG   = 'w3-light-grey w3-text-red'}
                    $str .= sprintf '<td class="w3-center w3-border %s">%d</td>', $scoreBG, $inScore;
                    push (@{$inSec.'_'.$inSub}, $inScore);
                }
            } 
            my $weightedScore = $SCORE{$cardIDX}{IN_SCORE};
            push(@WSCORE, $weightedScore);
            $str .= sprintf '<td class="w3-small" style="text-align: right;"><a href="javascript:void(0);" onclick="paper_showVolunteerStatistics(this, %d, %d, %d, %d);">%2.2f</a></td>', $userIDX, $classIDX, $teamIDX, $cardIDX, $weightedScore ;
            # $str .= sprintf '%2.2f</td>', $weightedScore;
            $str .= '</tr>';
        }
        $str .= '<tr class="w3-small w3-light-grey">';
        $str .= '<td><b>Std-Dev.</b></td>';
        foreach $inSec (sort {$a <=> $b} keys %RUBRIC) {
            foreach $inSub (sort {$a <=> $b} keys %{$RUBRIC{$inSec}}) {
                my $std = stddev(@{$inSec.'_'.$inSub});
                my $bgColor = 'w3-pale-green';
                if ($std >= 2.245){
                    $bgColor = 'w3-pale-red';
                    } elsif ($std >= 1.245) {
                    ...
                    $str .= sprintf '<td class="w3-center w3-border %s " style="width: 65px;">%2.2f</td>', $bgColor, $std;
            }
        } 
            $str .= sprintf '<td style="text-align: right;">%2.2f</td>', stddev(@WSCORE);
        $str .= '</tr>';
        $str .= '</tbody>';
        $str .= '</table>';
        $str .= '</div>';
    ...
    sub paper_openManagePapers(){
        ...
        $str .= sprintf '<button class="w3-bar-item w3-button paperTab " onclick="paper_openTab(this, %d);">Design Reports</button>', 1;
        $str .= sprintf '<button class="w3-bar-item w3-button paperTab " onclick="paper_openTab(this, %d);">TDS</button>', 2;
        $str .= sprintf '<button class="w3-bar-item w3-button paperTab " onclick="paper_openTab(this, %d);">Drawings</button>', 3;
        $str .= sprintf '<button class="w3-bar-item w3-button paperTab " onclick="paper_openTab(this, %d);">Requirements</button>', 4;
        $str .= '</div>';
        $str .= '<div id="tabContent"    class="w3-container w3-white" style="padding: 0px;"></div>';
        $str .= '</div>';
        # $str .= &paper_openManagePapers_Leg();
        return ($str);
        }
    sub paper_openTab (){
        print $q->header();
        ...
        my $str;
        if ($inType == 1){
            $str .= &paper_loadDesignPapers();
            } else {
                $str .= &paper_loadOthers();
            }
        return ($str);
        }
    sub paper_loadOthers(){
        my $eventIDX  = $q->param('eventIDX');
        my %HEADING   = (2=>'Technical Design Sheets (TDS)', 3=>'Drawings', 4=>'Requirements');
        my $inType    = $q->param('inType');
        my $Team      = new SAE::TEAM();
        my $Paper     = new SAE::PAPER();
        my $User      = new SAE::USER();
        my %TEAMS     = %{$Team->_getClassTeamList($eventIDX)};
        my %VOL       = %{$Paper->_getAssignedVolunteerList($eventIDX, $inType)};
        my %INIT      = %{$User->_getUserFirstInitialAndLastName()};
        my $str = sprintf '<h2>%s</h2>', $HEADING{$inType};
         foreach $classIDX (sort keys %CLASS) {
            $str .= '<div class="w3-container w3-border w3-round w3-margin-bottom w3-padding">';
            $str .= sprintf '<h3>%s Class</h3>', $CLASS{$classIDX};
            $str .= '<table class="w3-table-all">';
            $str .= '<thead>';
            $str .= '<tr>';
            $str .= '<th style="width: 40px;">#</th>';
            $str .= '<th style="width: 300px;">School</th>';
            # $str .= '<th rowspan="2" style="width: 66px;">Days<br>Late</th>';
            $str .= sprintf '<th>Judge <a class="w3-margin-left" href="javascript:void(0);" onclick="paper_openBatchAssign(this, %d, %d);">Batch-Assign</a></th>', $classIDX, $inType;
            $str .= '</tr>';

            $str .= '</thead>';
            $str .= '<tbody>';
            foreach $teamIDX (sort {$TEAMS{$classIDX}{$a}{IN_NUMBER} <=> $TEAMS{$classIDX}{$b}{IN_NUMBER}} keys %{$TEAMS{$classIDX}}) {
                my $txSchool   =  $TEAMS{$classIDX}{$teamIDX}{TX_SCHOOL};
                my $inNumber   =  $TEAMS{$classIDX}{$teamIDX}{IN_NUMBER};
                $str .= '<tr>';
                $str .= sprintf '<td>%03d</td>', $inNumber;
                $str .= sprintf '<td>%s</td>', $txSchool;
                # $str .= sprintf '<td >%s</td>';
                $str .= sprintf '<td ID="row_%d" nowrap>', $teamIDX;
                $str .= sprintf '<button class="w3-button w3-round w3-border" onclick="paper_openJudgeList(this, %d, %d, %d);">+</button>', $classIDX, $teamIDX, $inType;
                foreach $cardIDX (sort keys %{$VOL{$teamIDX}}) {
                    my $userIDX  = $VOL{$teamIDX}{$cardIDX}{FK_USER_IDX};
                    my $inStatus = $VOL{$teamIDX}{$cardIDX}{IN_STATUS};
                    my $volName  = $INIT{$userIDX};
                    $str .= $Paper->_getTemplateJudgeButton($cardIDX, $classIDX, $teamIDX, $inType, $volName, $inStatus);
                } 
                $str .= sprintf '</td>';
                $str .= '</tr>';
            }
    ...
    sub paper_loadDesignPapers(){
        ...
        my %VOL       = %{$Paper->_getAssignedVolunteerList($eventIDX, $inType)};
        my %INIT      = %{$User->_getUserFirstInitialAndLastName()};
        my %STATS     = %{$Paper->_getAllTeamAssessmentStatistics('Assessment')};
        my $str;
        $str = '<h2>Design Papers Assignments</h2>';
        ...
        foreach $classIDX (sort keys %CLASS) {
            ...
            $str .= '<thead>';
            $str .= '<tr>';
            $str .= '<th rowspan="2" style="width: 40px;">#</th>';
            $str .= '<th rowspan="2" style="width: 150px;">School</th>';
            $str .= '<th rowspan="2" style="width: 66px;">Days<br>Late</th>';
            $str .= sprintf '<th rowspan="2" style=" ">Judge<br><a href="javascript:void(0);" onclick="paper_openAutoAssign(this, %d, 1)">Auto-Assign</a></th>', $classIDX;
            $str .= '<th colspan="4" style="width: 150px">Statistics</th>';
            $str .= '</tr>';
            $str .= '<tr>';
            $str .= '<th style="width: 65px;">High</th>';
            $str .= '<th style="width: 65px;">Low</th>';
            $str .= '<th style="width: 65px;">STD</th>';
            $str .= '<th style="width: 65px;">Avg.</th>';
            $str .= '</tr>';
            $str .= '</thead>';
            $str .= '<tbody>';
            foreach $teamIDX (sort {$TEAMS{$classIDX}{$a}{IN_NUMBER} <=> $TEAMS{$classIDX}{$b}{IN_NUMBER}} keys %{$TEAMS{$classIDX}}) {
                my $txSchool =  $TEAMS{$classIDX}{$teamIDX}{TX_SCHOOL};
                my $inDays   =  $TEAMS{$classIDX}{$teamIDX}{IN_DAYS};
                $str .= '<tr>';
                $str .= sprintf '<td>%03d</td>', $TEAMS{$classIDX}{$teamIDX}{IN_NUMBER};
                $str .= sprintf '<td><a href="javascript:void(0);" onclick="paper_getTeamScoringDetails(this, %d, %d, %d);">%s</a></td>', $teamIDX, $inType, $classIDX, $txSchool;
                $str .= sprintf '<td><input type="number" class="w3-input w3-border w3-white" value="%d" ste="1" min="0" onchange="paper_updateDaysLate(this, %d);"></td>', $inDays, $teamIDX;
                $str .= sprintf '<td ID="row_%d" nowrap>', $teamIDX;
                $str .= sprintf '<button class="w3-button w3-round w3-border" onclick="paper_openJudgeList(this, %d, %d, %d);">+</button>', $classIDX, $teamIDX, 1; #1 = Design Report
                foreach $cardIDX (sort keys %{$VOL{$teamIDX}}) {
                    my $userIDX  = $VOL{$teamIDX}{$cardIDX}{FK_USER_IDX};
                    my $inStatus = $VOL{$teamIDX}{$cardIDX}{IN_STATUS};
                    my $volName  = $INIT{$userIDX};
                ...
                $str .= sprintf '</td>';
                $str .= sprintf '<td class="w3-border-left w3-small"><span class=" stat_max max_%d">%2.2f</span></td>', $teamIDX, $STATS{$teamIDX}{IN_MAX};
                $str .= sprintf '<td class="w3-small"><span class=" stat_min min_%d">%2.2f</span></td>', $teamIDX, $STATS{$teamIDX}{IN_MIN};
                $str .= sprintf '<td class="w3-small"><a href="javascript:void(0);" class="stat_std std_%d" onclick="paper_getTeamScoringDetails(this, %d, %d, %d);">%2.2f</a></td>', $teamIDX, $teamIDX, $inType, $classIDX, $STATS{$teamIDX}{IN_STD};
                $str .= sprintf '<td class="w3-small "><b class="stat_avg  avg_%d">%2.2f</b></td>', $teamIDX, $STATS{$teamIDX}{IN_AVG};
                $str .= '</tr>';
                $str .= sprintf '<tr ID="DETAILS_%d" class="w3-hide">', $teamIDX;
                $str .= sprintf '<td ID="DETAIL_CONTENT_%d" class="scoringDetails" style="padding: 0px;" colspan="8" ></td>', $teamIDX;
                $str .= '</tr>';
            }
            $str .= '</tbody>';
            $str .= '</table>';
    ...
    sub paper_batchAssign (){
        ...
        my $userIDX    = $q->param('userIDX');
        my $inType     = $q->param('inType');
        my $volName    = $q->param('volName');
        my $Paper      = new SAE::PAPER();
        my %BUTTON     = %{$Paper->_batchAssign($eventIDX, $classIDX, $userIDX, $inType, $volName)};
        if (%BUTTON){
                my $json = encode_json \%BUTTON;
                return ($json);
            } else {
    ...
    sub paper_openBatchAssign (){
        ...
        my $inType     = $q->param('inType');
        my $Paper      = new SAE::PAPER();
        my %VOL        = %{$Paper->_getOtherVolunteerList($eventIDX, $inType)};
        my $str;
        $str .= '<div class="w3-container" style="height: 400px; overflow-y: auto;">';
        $str .= '<div class="w3-column-3">';
        $str .= sprintf '<div class="list_item">';
        $str .= sprintf '<input ID="0" type="radio" value="0" Name="batch" class="w3-check">';
        $str .= sprintf '<label FOR="0" class="w3-margin-left w3-text-red"><i> --Un-assigned-- </i></label>';
        $str .= sprintf '</div>';
        foreach $volIDX (sort {lc($VOL{$a}{TX_LAST_NAME}) cmp lc($VOL{$b}{TX_LAST_NAME})} keys %VOL) {
            my $volName     = $VOL{$volIDX}{TX_INIT};
            my $volFullName = sprintf '%s, %s', $VOL{$volIDX}{TX_LAST_NAME}, $VOL{$volIDX}{TX_FIRST_NAME};
            $str .= sprintf '<div class="list_item">', $VOL{$volIDX}{TX_LAST_NAME}, $VOL{$volIDX}{TX_FIRST_NAME};
            $str .= sprintf '<input ID="%d" type="radio" Name="batch" data-value="%s" value="%d" class="w3-check batch">', $volIDX, $volName, $volIDX;
        ...
        $str .= sprintf '<button class="w3-center w3-button w3-round w3-border w3-pale-green w3-hover-green" onclick="paper_batchAssign(this, %d, %d);">Batch Assign</button>', $classIDX, $inType;
        $str .= '</div>';
        return ($str);
        }
    sub paper_openJudgeList (){
        print $q->header();
        my $User       = new SAE::USER();
        my $eventIDX   = $q->param('eventIDX');
        my $classIDX   = $q->param('classIDX');
        ...
        foreach $volIDX (sort {lc($VOL{$a}{TX_LAST_NAME}) cmp lc($VOL{$b}{TX_LAST_NAME})} keys %VOL) {
            my $checked = '';
            my $cardIDX = 0;
            if (exists $ASSIGNED{$volIDX}){
                $checked = 'checked';
                $cardIDX = $ASSIGNED{$volIDX}{PK_CARD_IDX};
            }
            # my $volName  = $INIT{$volIDX};
            my $volName     = $VOL{$volIDX}{TX_INIT};
            my $volFullName = sprintf '%s, %s', $VOL{$volIDX}{TX_LAST_NAME}, $VOL{$volIDX}{TX_FIRST_NAME};
            $str .= sprintf '<div class="list_item">', $VOL{$volIDX}{TX_LAST_NAME}, $VOL{$volIDX}{TX_FIRST_NAME};
            $str .= sprintf '<input type="checkbox" class="w3-check" %s value="%d" data-value="%s" onclick="paper_addRemoveAssignment(this, %d, %d, %d, %d);">', $checked, $volIDX, $volName, $teamIDX, $cardIDX, $inType, $classIDX;
            $str .= sprintf '<label class="w3-margin-left">%s</label>', $volFullName;
            $str .= sprintf '</div>';
        }
        $str .= '</div>';
        $str .= '</div>';
    ...
    sub paper_addRemoveAssignment (){
        print $q->header();
        my $eventIDX      = $q->param('eventIDX');
        my $addVolunteer  = $q->param('addVolunteer');
        my $cardIDX       = $q->param('cardIDX');
        my $teamIDX       = $q->param('teamIDX');
        my $inType        = $q->param('inType');
        my $userIDX       = $q->param('userIDX');
        ...
        my $Paper         = new SAE::PAPER();
        if ($addVolunteer == 1) {
            my $newCardIDX = &paper_assignVolunteerJudge($userIDX, $teamIDX, $eventIDX, $inType, $classIDX );
            # $str = &paper_templateJudgeButton($newCardIDX, $classIDX, $teamIDX, $inType , $volName, 0);
            $str .= $Paper->_getTemplateJudgeButton($newCardIDX, $classIDX, $teamIDX, $inType , $volName, 0);
            return ($str);
        } else {
            &paper_removeVolunteerJudge($cardIDX);
            return($cardIDX);
        }
        return;        
        }
    sub paper_assignVolunteerJudge(){
        my ($userIDX, $teamIDX, $eventIDX, $inType, $classIDX) = @_;
        my $Paper         = new SAE::PAPER();
        my $newcardIDX    = $Paper->_assignTeamToJudge($userIDX, $teamIDX, $eventIDX, $inType, $classIDX);
        return ($newcardIDX);
        }
    sub paper_removeVolunteerJudge(){
        my ($cardIDX) = @_;
        my $JsonDB  = new SAE::JSONDB();
        $JsonDB->_delete('TB_CARD',qq(PK_CARD_IDX=$cardIDX));
        return ();
        }
    sub paper_updateDaysLate (){
        my $eventIDX   = $q->param('eventIDX');
        my $inDays     = $q->param('inDays'); 
        my $teamIDX    = $q->param('teamIDX');
        my $Paper      = new SAE::PAPER();
        $Paper->_saveDaysLate($inDays , $teamIDX);
        return ();
        }
#= 2024 =============================================================================
...
#     return ($str);
#     }
# sub paper_openManagePapers_Leg(){
#     # print $q->header();
# 	my $eventIDX   = $q->param('location');
#     $str .= '<div class="w3-container w3-margin-top">';
#     $str .= '<br><h2>Manage Papers</h2>';
