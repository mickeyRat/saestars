...
exit;
# ================= 2024 ==============================
sub student_unlinkTeam(){
    print $q->header();
    $userIDX   = $q->param('userIDX');
...
sub student_linkTeam(){
    print $q->header();
    my $inCode         = $q->param('inCode');
    my $Team           = new SAE::TEAM();
    my %DATA           = %{decode_json($q->param('jsonData'))};
    my $teamIDX        = $Team->getTeamIndex($inCode);
    $DATA{FK_TEAM_IDX} = $teamIDX;
    if ($teamIDX){
            my $JsonDB         = new SAE::JSONDB();
            my $newIDX         = $JsonDB->_insert('TB_USER_TEAM', \%DATA);
            return(1);
        } else {
            return(0);
        }
    
    # return ("$teamIDX, $newIDX");
    }
# ================= 2022 ==============================
...
sub student_openSafetyChecks (){
    ...
    my %STATUS     = (''=>'Not Started', 0=>'Failed', 1=>'Passed');
    # my %COLOR       = (''=>'w3-text-black', 0=>'w3-text-black', 1=>'w3-text-black', 2=>'w3-text-red', 3=>'w3-text-blue');
    my %BGCOLOR     = (''=>'w3-light-grey', 0=>'w3-pale-red', 1=>'w3-light-blue');
    my $str;
    ...
    foreach $headingIDX (sort {$HEAD{$a}{IN_SECTION} <=> $HEAD{$b}{IN_SECTION}} keys %HEAD) {
        $str .= '<div class="w3-container w3-row w3-light-grey w3-border w3-margin-bottom">';
        $str .= sprintf '<h4><b>%d - %s</b></h4>', $HEAD{$headingIDX}{IN_SECTION}, $HEAD{$headingIDX}{TX_SECTION};
        foreach $itemIDX (sort {$LIST{$headingIDX}{$a}{IN_SECTION} <=> $LIST{$headingIDX}{$b}{IN_SECTION}} keys %{$LIST{$headingIDX}}) {
            ...
            my $inSection = $LIST{$headingIDX}{$itemIDX}{IN_SECTION};
            my $txSection = $LIST{$headingIDX}{$itemIDX}{TX_SECTION};
            my $checked = '';
            my $leftBar = 'w3-border-red';
            if ($LIST{$headingIDX}{$itemIDX}{BO_CHECK} == 1){$checked = 'checked'; $leftBar = 'w3-border-green'}
            $str .= sprintf '<label for="ITEM_%d" class="w3-col l12 w3-border w3-round w3-padding w3-white w3-leftbar %s" style="margin: 2px 0px;">', $itemIDX, $leftBar;
            # $str .= sprintf '<label for="ITEM_%d">', $itemIDX;
            $str .= sprintf '<input ID="ITEM_%d" %s class="w3-check " data-field="BO_CHECK" data-index="%d" type="checkbox" onchange="student_updateCheckItem(this, %d);">',$itemIDX, $checked, $itemIDX, $teamIDX;
            $str .= sprintf ' <span class="w3-margin-left">%d.%d - %s</span>', $inHeading, $inSection, $txSection;
            $str .= '</label>';
        }
        $str .= '</div>';
    }
...
# ------------------
    ...
    #             $str .= sprintf '<label  for="ITEM_'.$itemIDX.'" class="w3-margin-left" >%d.%s - %s</label><br>',$inHeading, $LIST{$headingIDX}{$itemIDX}{IN_SECTION},$LIST{$headingIDX}{$itemIDX}{TX_SECTION};
...
sub student_openRequirementsChecks (){
    ...
    my %LIST        = %{$Student->_getListOfSafetyItems($teamIDX, $classIDX, $txType)};
    my %TECH        = %{$Tech->_getTeamTechList($teamIDX)};
    my %STATUS     = (''=>'Not Started', 0=>'Failed', 1=>'Passed', 2=>'Pass w/Penalties');
    # my %COLOR       = (''=>'w3-text-black', 0=>'w3-text-black', 1=>'w3-text-black', 2=>'w3-text-red', 3=>'w3-text-blue');
    my %BGCOLOR     = (''=>'w3-light-grey', 0=>'w3-pale-red', 1=>'w3-light-blue', 2=>'w3-pale-yellow');
    my $str;
    $str .= '<div class="w3-container" style="overflow-y: auto;">';
    # $str .= '<h5 class="w3-container w3-border w3-round w3-border-red w3-pale-yellow w3-padding">';
    $str .= '<div class="w3-panel w3-pale-yellow w3-padding w3-border w3-border-red">';
    $str .= '<b>INSTRUCTION:<br>To self-Certify, click on the checkbox provided below for each inspection item.</b><br>';
    $str .= 'By checking off each requirement, the team certifies that the requirement is 100% in compliance with the current rules.';
    $str .= '<p>Please reference [ Section 6.6 REQUIREMENTS CHECK & SAFETY AND AIRWORTHINESS INSPECTION PENALTIES ] in the rules document for further details</p>';
    $str .= '<b><span class="w3-text-red"><b>*</b></span> Abosolute Requirements: (AbsReq)</b> <i>Team must satisfy all Absolute Requirements to participate in the flying portion of the event.</i><br>';
    ...
    foreach $headingIDX (sort {$HEAD{$a}{IN_SECTION} <=> $HEAD{$b}{IN_SECTION}} keys %HEAD) {
        my $inHeading = $HEAD{$headingIDX}{IN_SECTION};
        my $txHeading = $HEAD{$headingIDX}{TX_SECTION};
        $str .= sprintf '<div class="w3-row w3-rest w3-border w3-padding w3-leftbar w3-round-large w3-margin-bottom">';
        $str .= sprintf '<b>%d - %s</b>', $inHeading , $txHeading ;
        foreach $itemIDX (sort {$LIST{$headingIDX}{$a}{IN_SECTION} <=> $LIST{$headingIDX}{$b}{IN_SECTION}} keys %{$LIST{$headingIDX}}) {
            my $inPoints   = $LIST{$headingIDX}{$itemIDX}{IN_POINTS};
            my $inSection  = $LIST{$headingIDX}{$itemIDX}{IN_SECTION};
            my $txSection  = $LIST{$headingIDX}{$itemIDX}{TX_SECTION};
            my $inAbsolute = $LIST{$headingIDX}{$itemIDX}{BO_ABSOLUTE};
            my $checked = '';
            if ($LIST{$headingIDX}{$itemIDX}{BO_CHECK} == 1){$checked = 'checked'}
            $str .= sprintf '<div class="w3-rest w3-border-0 w3-padding w3-white">';
            # $str .= "$itemIDX, $checked, $itemIDX, $teamIDX";
            $str .= sprintf '<input ID="ITEM_%d" %s class="w3-check w3-margin-right w3-xlarge" data-field="BO_CHECK" data-index="%d" type="checkbox" onchange="student_updateCheckItem(this, %d);">', $itemIDX, $checked, $itemIDX, $teamIDX;
            $str .= sprintf '<label for="ITEM_%d">', $itemIDX;
            if ($inAbsolute == 1) {
                $str .= sprintf '<span class="w3-text-red"><b>*</b></span> %d.%d (%2.1f <span class="w3-small">pts.</span>) - [<span class="w3-text-red">AbsReq</span>] %s', $inHeading, $inSection, $inPoints, $txSection;
            } else {
                $str .= sprintf '%d.%d (%2.1f <span class="w3-small">pts.</span>) - %s', $inHeading, $inSection, $inPoints, $txSection;
            }
            $str .= sprintf '</label>';
            $str .= sprintf ' </div>';
        }
