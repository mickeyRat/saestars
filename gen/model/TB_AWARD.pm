...
sub addNewRecord(){
	...
	my $SQL = "INSERT INTO TB_AWARD (FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION) values (?, ?, ?, ?)";
	my $insert = $dbi->prepare($SQL);
	   $insert->execute($FkClassIdx, $InOrder, $TxTitle, $TxDescription);
...
sub getRecordById(){
	my ($self) = shift;
	my ($idx) = shift;
	$SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION
		FROM TB_AWARD
		WHERE PK_AWARD_IDX=?";
...
sub getAllRecordBy_PkAwardIdx(){
	...
	my $SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION FROM TB_AWARD WHERE PK_AWARD_IDX=?";
	my $select = $dbi->prepare($SQL);
...
sub getAllRecordBy_FkClassIdx(){
	...
	my $SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION FROM TB_AWARD WHERE FK_CLASS_IDX=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_AWARD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_InOrder(){
	...
	my $SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION FROM TB_AWARD WHERE IN_ORDER=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_AWARD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxTitle(){
	...
	my $SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION FROM TB_AWARD WHERE TX_TITLE=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_AWARD_IDX'])};
	return(\%HASH);
...
sub getAllRecordBy_TxDescription(){
	...
	my $SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION FROM TB_AWARD WHERE TX_DESCRIPTION=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my %HASH = %{$select->fetchall_hashref(['PK_AWARD_IDX'])};
	return(\%HASH);
...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_AWARD_IDX, FK_CLASS_IDX, IN_ORDER, TX_TITLE, TX_DESCRIPTION FROM TB_AWARD";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_AWARD_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_TxDescription(){
	...
	my $SQL = "DELETE FROM TB_AWARD where TX_DESCRIPTION=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub getIdBy_TxDescription(){
	...
	$SQL = "SELECT PK_AWARD_IDX from TB_AWARD where TX_DESCRIPTION=?";
	my $select = $dbi->prepare($SQL);
	   $select->execute($value);
	my $idx = $select->fetchrow_array();
...
sub updateTxDescription_byId(){
	...
	my $SQL = "UPDATE TB_AWARD set TX_DESCRIPTION=? where PK_AWARD_IDX=?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return($value);
