...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_JUDGE_TEAM_IDX, FK_EVENT_IDX, FK_TEAM_IDX, FK_USER_IDX, TX_TYPE, FK_ASSESSMENT_TYPE_IDX FROM TB_JUDGE_TEAM";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_JUDGE_TEAM_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_PkJudgeTeamIdx(){
	my ($self) = shift;
	my ($idx) = shift;
...
sub deleteRecordBy_FkEventIdx(){
	...
	my $SQL = "DELETE FROM TB_JUDGE_TEAM where FK_EVENT_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_FkTeamIdx(){
	...
	my $SQL = "DELETE FROM TB_JUDGE_TEAM where FK_TEAM_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_FkUserIdx(){
	...
	my $SQL = "DELETE FROM TB_JUDGE_TEAM where FK_USER_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxType(){
	...
	my $SQL = "DELETE FROM TB_JUDGE_TEAM where TX_TYPE=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_FkAssessmentTypeIdx(){
	...
	my $SQL = "DELETE FROM TB_JUDGE_TEAM where FK_ASSESSMENT_TYPE_IDX=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub updatePkJudgeTeamIdx_ById(){
	my ($self) = shift;
	my ($value) = shift;
	my ($idx) = shift;

	my $SQL = "UPDATE TB_JUDGE_TEAM SET PK_JUDGE_TEAM_IDX=? where PK_JUDGE_TEAM_IDX = ?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return ();
}
