...
sub getAllRecord(){
	...
	my $SQL = "SELECT PK_SECTION_IDX, IN_SECTION_REF, TX_SECTION, IN_WEIGHT FROM TB_SECTION_REF";
	my $select = $dbi->prepare($SQL);
	   $select->execute();
	my %HASH = %{$select->fetchall_hashref(['PK_SECTION_IDX'])};
	return(\%HASH);
...
sub deleteRecordBy_InSectionRef(){
	my ($self) = shift;
	my ($idx) = shift;

	my $SQL = "DELETE FROM TB_SECTION_REF where IN_SECTION_REF=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_TxSection(){
	...
	my $SQL = "DELETE FROM TB_SECTION_REF where TX_SECTION=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub deleteRecordBy_InWeight(){
	...
	my $SQL = "DELETE FROM TB_SECTION_REF where IN_WEIGHT=?";
	my $delete = $dbi->prepare($SQL);
	   $delete->execute($idx);
	return();
}
...
sub updatePkSectionIdx_ById(){
	...
	my $SQL = "UPDATE TB_SECTION_REF SET PK_SECTION_IDX=? where PK_SECTION_IDX = ?";
	my $update = $dbi->prepare($SQL);
	   $update->execute($value, $idx);
	return ();
}
